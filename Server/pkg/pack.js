...
archive.pipe(fs.createWriteStream(path.join(dist, '/windows_32bit_debug.zip')));
archive.file(path.join(dist, 'server_win_32bit_debug.exe'), { name: 'server_win_debug.exe' });
archive.file(path.join(__dirname, 'startup_windows.bat'), { name: 'startup_windows.bat' });
archive.finalize();
...
archive.file(path.join(dist, 'server_macos_no_tray_debug'), { name: 'server_macos_no_tray_debug' });
archive.finalize();
